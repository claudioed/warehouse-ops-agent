...
detector, adapted from inventory-storage's harness-coverage-expansion plan
(Phase 5 Task 5.1) for warehouse-ops-agent's smaller domain shape.
...
blocks a PR. warehouse-ops-agent's whole domain
(./internal/domain/policy) is small enough that CI's blocking
`mutation-fast` job already runs gremlins over the SAME
`./internal/domain` tree on every push/PR (see .gremlins.yaml), so this
script simply re-uses that command's output rather than a separate
fast/full split.
...
def main() -> int:
    ...
    print(
        "\nCross-check each against known accepted survivors before treating "
        "as a real gap. This job is advisory: it does not fail the build."
    )
    return 0
